                ...
                for (i = 0; i < RAYCOUNT; i++)
                {
                    if (reflectionPoints[j].ContainsKey(i) && reflectionPoints[j + 1].ContainsKey(i))
                    {
                     ...
                     //   Vector3 vec3Aux;
                        ...
                        hs = Physics.RaycastAll(r, Mathf.Abs(Vector3.Distance(spoint, reflectionPoints[j + 1][i])), Inversor);
    ...
    void CreateMesh()
    {
        ...
        if (canLight)
        {
            ...
            for (i = 0; i < RAYCOUNT; i++)
            {
                ...
                if (Physics.Raycast(r, out h, Mathf.Abs(RAYLENGTH), Mirror))
                {
                    //De global pra local
